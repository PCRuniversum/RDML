...
This vignette descripts how to create **RDML** object from user provided data and calculate Cq values with custom **RDML** class method.

For the this purpose we used data from the VideoScan platform. The VideoScan platform is a highly versitile fluorescence microscope based imaging platform for the real-time analysis of biomoloecular interactions. VideoScan has been used for DNA meltig curve analysis [@roediger_RJ_2013], real-time quantitative PCR [@roediger_highly_2013] and quantitative isothermal amplification [@spiess_impact_2014].

We will use **C54** data from the [chipPCR](http://cran.r-project.org/web/packages/chipPCR/index.html) package [@roediger2015chippcr] -- a qPCR experiment for the amplification of *MLC-2v* performed using the *VideoScan* heating/cooling-unit. This data contains three amplification curves collected by three runs. Amplification curves have different number of cycles and one of them contains missing value. Thus, this data should be preprocessed before Cq calculation.
...
As optional step we can add some information about our experiment. Such as experimentators contacts, references to documentation, description of samples and targets, thermal cycling conditions, etc.
