...
that link with the **RMDL** package. It will give a examples how 
...
In principle, the following code examples can be combined with report generation 
...
# Extraction of the amplification curve data
...
# Plotting of the raw amplification curve data
...
of defualt **R** graphics functions. 
...
use the second derivative maximum as Cq value for further calculations. This 
supplementary material will not focus on the selection of a Cq method. For an 
...
Following we selected arbitarily a non-template control (negative) and an 
unknown sample (positive) (two out of `r ncol(fdata)-1` amplification curves) 
for the presentation of the coefficients that were calcuaed from the amplication 
curve data.
...
# Calculation of the amplication efficiency from the calibration data

We used the our calculated Cq values of the dilutions steps (`r 625*c(1, 2, 4, 8, 
16)`) to determine the calibration curve. According to the StepOne manufacturers 
software is the amplification efficiency circa 
`r round(raw_data$target[["RNase P"]]$amplificationEfficiency, 1)`%. 
To test this, the `effcalc` function was used to determine the coefficients of the 
calibration curve.

The following few lines are needed to calculate the amplication efficiency based on 
Cq values that were calculated before.
...
Coefficient of Variance (RSD [%]) are presented. The results indicate that the 
