...
Package **RDML** is created to work with Real-time PCR Data Markup Language (RDML) -- a structured and universal data standard for exchanging quantitative PCR (qPCR) data. RDML belongs to the family of eXtensible Markup Languages (XML).  It contains fluorescence data and information about qPCR experiment overall. Description and schema of RDML format can be found at http://rdml.org. 

Structure of **RDML** package mimics RDML format and provides several [R6](http://cran.r-project.org/web/packages/R6/vignettes/Introduction.html) classes which corresponds to [RDML v1.2](http://rdml.org/files.php?v=1.2) format types . All major manipulations with RDML data can be done by class called **RDML** throught its public methods:
...
To open this file we have to create new **RDML** object with its class initializer -- `$new()` and file name as parameter `filename`. 
```{r, results = "hide"}
...
lc96
```
As a result we can see fields names and after `:` :
...
Contains one or more experiments with fluorescence data. Fluorescence data store at the *data* level of experiment. E.g., fluorescence data for reaction tube *45* and target *bACT* can be accessed with the next code:
```{r}
fdata <- 
  lc96$
...
At this dendrogram we can see that our file consists of one experiment with one run in it. Four targets, each with two sample types (*std* -- standard, *unkn* -- unknown). There is only qPCR data -- *adp*. Ten reacts (tubes) for standard type and six reacts for unknown type. Total number of reacts can be more than number of reacts on plate because one tube can contain several targets.

### Additional information fields

All fields other other than **experiment**. This additional information can be referenced in other parts of file. E.g., to access sample added to react *39* and get its quantity we can use code like this:
```{r}
ref <- lc96$
          experiment$`ca1eb225-ecea-4793-9804-87bfbb45f81d`$
          run$`65aeb1ec-b377-4ef6-b03f-92898d47488b`$
...
**R6** objects are environments, that's why simple copying results in creating reference to existing object. Then modifing of copy leads to modification of original object. To create *real* copy of object we have to use method `$clone(deep = TRUE)` provided by **R6** class.
...
As we can see from plot our curves are not background substructed. To do this we will use function `CPP()` from package [chipPCR](https://github.com/michbur/chipPCR).
...
fdata.cpp <- cbind(cyc = fdata[, 1],
                   apply(fdata[, -1], 2,
                         function(x) CPP(fdata[, 1],
                                         x)$y))
```
Now we have preprocessed data which we'll add to our object and use during next section.
...
To provide functional programming style which is more convinient in **R** **RDML** class methods have function wrappers:
...
**RDML** package provide classes and methods to work with RDML data generated by real-time PCR devices or create RDML files from user generated data. Cause classes of **RDML**  package are build with **R6** they can be easy modified by adding custom methods and suggest type safe usage by input validation.