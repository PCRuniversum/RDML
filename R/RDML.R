...
InitRDML <- function(self, private, file.name) {
  ...
  private$.plate.map <- data.frame(ReactID = reacts.ids,                                                    
                                   Tube = tubes,
                                   TubeName = tube.names,
                                   Target = targets,
                                   Type = types,
...
#' \code{chipPCR} packages (see \link{new.RDML}). Real-time PCR Data Markup 
...
#' 
#' Possible keys in \code{name.pattern} \describe{ 
#'   \item{\code{\%NAME\%}}{name of the sample inputted in the qPCR software
#'   (ex.: "Sample 1")} \item{\code{\%ID\%}}{tube ID (ex.: "23")}
#'   \item{\code{\%TUBE\%}}{tube name (ex.: "B11")} \item{\code{\%TARGET\%}}{PCR
#'   target (ex.: "GAPDH")} \item{\code{\%TYPE\%}}{type of the sample (ex.:
#'   "unkn")}  }
#' 
#' @name RDML.class
...
#'   -- \emph{READ ONLY}} \item{dilutions}{\code{list} of dillutions with their 
#'   positions at plate splitted by targets } \item{plate.dims}{plate dimensions
#'   -- number of rows and columns at plate} \item{targets}{targets (genes or 
#'   dyes names) used by experiment} \item{types}{ \code{list} of samples types 
#'   splitted by targets} \item{name.pattern}{ name pattern to generate 
#'   fluorescence data vectors names (see 'Details') } }   
...
#' lc96$SetTargets(c("famatbact", "hexatx", "texasredaty", "cy5atipc"))
#' ## Show types of the samples for target 'famatbact'
#' lc96$types("famatbact")
#' 
...
RDML <- R6Class("RDML",
                public = list(
                  ...
                  GetFData = function(filter = list(method = "qPCR")) {
                    ...
                    if("react.ids" %in% params.names)
                      ...
                      filtered.indices <- 1:nrow(private$.plate.map)
                      if("targets" %in% params.names)
                        filtered.indices <- which(
                          private$.plate.map$Target %in% filter$targets)
                      if("types" %in% params.names)
                        filtered.indices <- which(
                          private$.plate.map$Type[filtered.indices] 
                          %in% filter$types)
                      if("names" %in% params.names)
                        filtered.indices <- grep(filter$names, 
                                                 private$.plate.map$FDataName[filtered.indices])
                    }
                    
                    if(filter$method == "melt")
                      filtered <- as.data.frame(private$.melt.fdata[,filtered.indices])
                    else
                      filtered <- as.data.frame(
                        private$.qPCR.fdata[,filtered.indices],
                        row.names = rownames(private$.qPCR.fdata))
                    names(filtered) <- private$.plate.map$FDataName[
                      filtered.indices]
                    return(filtered)
                  }
                ),
                ...
                active = list(
                  ...
                  plate.dims = function() {private$.plate.dims},  
