...
#' 
#' File format options: \describe{ \item{auto} {Tries to detect format by extension. \code{.xlsx} -- \code{excel}, \code{.xls} -- \code{abi}, 
...
RDML$set("public", "initialize", function(filename,
  ...
  fromABI <- function() {
    ...
    mydescr <- data.frame(
      ...
      exp.id = "exp1",
      run.id = "run1",
      react.id = sapply(descr$Well, FromPositionToId),
      sample = descr$`Sample Name`,
      target = descr$`Target Name`,
      target.dyeId = descr$Reporter,
      quantity = descr$Quantity,
      type = descr$Task,
      stringsAsFactors = FALSE
    )
    mydescr <- mydescr %>% 
      filter(!is.na(target.dyeId)) %>% 
      group_by(fdata.name) %>% 
      mutate(sample = if (is.na(sample)) fdata.name
             else sample,
             type = switch (type,
                            NTC = "ntc",
                            STANDARD = "std",
      ...
      ungroup
    mypcrdata <- pcrdata %>% 
      gather("Dye", "Fluo", 3:ncol(pcrdata)) %>% 
      unite(Well_Dye, Well, Dye) %>% 
      spread(Well_Dye, Fluo) %>% 
      as.data.frame
