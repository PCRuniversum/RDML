                        ...
                        snames=NA) 
{
  ...
  # add first column with cycles
  output <- matrix(ncol=100)
  print(str(output))
  i <- 2
  output[1] <- as.vector(object[[runtype]][[targets[1]]][[types[1]]][[1]])
  for(fluorTarget in targets)
  {
    for(fluorType in types)
    {      
      if(is.na(snames)) {
        n <- length(names(object[[runtype]][[fluorTarget]][[fluorType]])) - 1
        print(n)
        r <- object[[runtype]][[fluorTarget]][[fluorType]][[-1]]
        print(length(r))
        x <- i:(i+n)
        print(x)
        print(str(output))
